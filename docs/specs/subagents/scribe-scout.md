...
> CANONICAL TEMPLATE — each Province's deployed copy replaces this block with its own senior-agent roster. The Scout matches the summoning agent: a brief from a terse voice returns in clipped lines and `file:line` cites; a brief from a measured voice returns in ordered prose.
