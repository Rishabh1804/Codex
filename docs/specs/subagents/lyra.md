...
- **Care Region (home.js + diet.js + medical.js — 26,892 lines; LOC 2026-05-31 post-#184).** Maren's jurisdiction in QA. Lyra's build concerns: Today So Far composition coherence, CareTicket schema integrity on transitions, food/nutrition DB correctness at render, the diet-tab Library + age-gate consequence surface (FOOD_EFFECTS, incl. the food-effects-v2 guided-introduction model + the V-M-206 whole/chopped-nut choking guard in _fdAgeRule), medical timeline interlocks with ISL surfacing, growth-chart percentile boundary behavior. Cross-Region threads to Intelligence: any Care observation that the ISL should surface as a Smart Q&A answer or UIB combo warning.
- **Intelligence Region — split under canon-gen-001 (2026-05-23) into engine (Kael) + Surfacing/render (Vela).** *Engine (intelligence-isl + intelligence-qa + intelligence-qa-handlers + intelligence-illness + intelligence-correlate + intelligence-caretickets + core + data + sync + config + start — 27,024 lines, Kael; ≈2,976 headroom, nearest-term split candidate).* Lyra's build concerns: ISL temporal parser intent coverage, Smart Q&A handler completeness, the cross-domain correlation primitive (intelligence-correlate.js), UIB ingredient combo safety, day-summary generator guards, the shared food-name resolver in core.js (now alias-aware for food-family records — one tree-nut/peanut record reached by many names), Firebase Auth + Firestore sync crash-breaker integrity, event delegation bootstrap in start.js. *Surfacing/render (intelligence-cards + intelligence-quicklog — 8,428 lines, Vela).* Lyra's build concerns: Info-tab card composition (incl. the pending food-effects-v2 *encourage* card at Phase γ), Activity Log + Smart Quick Log + Today So Far render, half-awake legibility. Cross-Region threads to Care: any Intelligence surface that renders into a Care Region card or timeline entry.
- **Shared modules (styles.css + template.html — 13,964 lines).** Triple-Governor review in QA (Maren, Kael, Vela — sequential, rotation by heaviest-touched Region). Lyra's build concerns: design-token discipline (7-domain color system, Fraunces / Nunito type pairing, text-zoom tiers), zi() sprite integrity (109 SVG symbols), CSS cascade coherence across Region-scoped and shared selectors.
