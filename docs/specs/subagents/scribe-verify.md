...
> CANONICAL TEMPLATE — each Province's deployed copy replaces this block with its own senior-agent roster, build commands, and test entry points, so the Verify Scribe runs the right checks without being told them each time.
