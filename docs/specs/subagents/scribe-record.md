...
> CANONICAL TEMPLATE — each Province's deployed copy replaces this block with its own senior-agent roster and the artifact conventions the Province keeps, so the record stub matches the shape the Chronicler expects.
