...
| — | hermetic-floor-doesnt-substitute-for-production-floor | Phase 3 native | 9+ sustainments (Phase 3 ratification + Phase 4 per-PR + per-sub-phase-close scope expansion at PR-33 + PR-38 + post-PR-39 Sovereign-floor catches) | Standard 3/3 |
...
- **a-null-from-a-byte-level-instrument-licenses-nothing-about-content** (SEP-Ops `soma-internal` 2026-08-29 data ingest, cross-cluster per the PR #74 precedent; Cipher jurisdiction. Two WhatsApp exports of one chat had to be tested for continuity. The byte-prefix test — `new.startswith(old)` — had correctly proved the previous pair cumulative, and returned **False** here. Read as written it says the historical record was altered. A line-level diff found **16 non-equal blocks, of which 14 are one-minute timestamp rendering shifts from a newer app version** — same sender, same body, stamped a minute earlier — and the other two are pure appends. **The record is content-identical over the overlap.** Independent confirmation: per-`(day, sender)` message counts across the 79 shared days of a second, differently-formatted export of the same chat return **zero mismatches**. Related to but distinct from `stale-branch-triage-requires-test-merge-not-endpoint-diff` (2026-08-03): that concerns misreading a diff's *magnitude*; this concerns a boolean instrument whose negative carries no content claim at all. **Rule: an equality test over bytes answers a question about bytes. Before reporting a record as changed, diff it and classify what actually differs.**)

- **a-separator-is-part-of-the-string-so-a-null-on-one-spelling-is-a-null-about-that-spelling** (SEP-Ops `soma-internal` 2026-08-29; Cipher jurisdiction. Sweeping seven SKU tokens from a shop relay against a 51,592-string corpus returned **six zeros**. Re-run with spacing, hyphen and abbreviation variants, **four of the seven were present all along**: `C CLAMP`→`C-CLAMP` 176 hits · `TAAR`→`TAR` 18 · `90CD`→`90 CD` 152 · `40CD`→`40 CD` 402. Publishing the naive pass would have created four phantom new SKUs — the exact failure mode that repository's own 2026-07-28 session committed and codified against. **Third instance in one session of the same shape**, the other two being the byte-prefix null above and a `grep -E` pattern written `A\|B`, where `\|` is a **literal pipe rather than alternation**, which returned NO ROW for three customers that are all in the alias file — including the one row that repository has failed on three separate occasions. **Rule: a zero is a fact about the pattern, not about the world. Vary the separator, the case and the escape dialect before reporting an absence — and name which you varied.**)

- **a-fair-copy-normalises-exactly-the-marks-being-counted** (SEP-Ops `soma-internal`, first instanced 2026-08-28, **second instance 2026-08-29** — counter-eligible under §B on the next observation. First: a weekly attendance file had transcribed a raw relay's literal `Extra---3 hours` tags into man-hour prose; a sweep of the weekly returned nothing and the absence was published as a fact about the shop's records. Second: eighteen register photographs arrived inline in a chat session and **could not be written to disk**, so the repository holds a hand transcription with no byte-preserved original behind it — structurally the same object as the weekly, and it now carries load-bearing figures including a payment sheet that contradicts the relay on four lines. **Rule: date and name the surface you swept. A transcription is evidence of a transcription; when it is the only artifact, say so in the artifact itself and send anything that turns on a digit back to the original.**)
...
- ⚠ **SEP-Ops (`soma-internal`) eligibility is OPEN, and three entries now sit in the watch-list under it (2026-08-29).** They landed on the **PR #74 precedent** — cross-cluster Aurelius invocation surfacing first-instance observations — **which is precedent, not ratification.** Constitution Book: Cluster A = Codex + SproutLab + MSc, Cluster B = SEP Invoicing + SEP Dashboard; **`soma-internal` is named in neither**, though it is the operational codex behind Cluster B's invoicing repo and runs its own Builder + four-Governor model. **Two questions for the Consul**: (1) does SEP-Ops sit in Cluster B, or is it a Province of its own shape? (2) may a SEP-Ops second-instance advance a counter here, or only a Codex/SproutLab one? **Until answered, treat these three as observational only — do NOT advance a counter on them.**
