...
**Last update:** **2026-08-10** (Codex/CLAUDE.md carrier refresh + first Cluster-B cross-cluster carry: 2 new watch-list entries — negative-sweep licensing, rendered-output verification as its own jurisdiction. Also refines `state-carrier-decay-is-misinformation-not-staleness`: the 2026-08-03 fix swept `memory.md` and left the same 2026-05-06 state live in `CLAUDE.md`, the file that actually auto-loads — **a carrier-decay fix is not done until every auto-loaded surface is swept.**)
...
- **a-negative-sweep-licenses-not-found-never-does-not-exist** (Cluster B / `soma-internal` 2026-08-10, ratified into that repo's `CLAUDE.md`; carried here 2026-08-10 as cross-cluster. **Four blockers in a single SEP-Ops session were claims of *exhaustiveness*, not lookup errors** — *"eight partNumbers are gauge-priced"* · *"the split held with no exception"* · *"a single-SKU account"* · *"the entry lag did not recur"* — each asserted after a partial look and refuted by data already on disk. The sharpest: a sweep that returned nothing was published as *"neither is Mehta or Dilip"* while `customer-aliases.md:204` carried the exact string in the Dilip section all along. **Rule: before writing *every / all / none / no exception / only / never / the first*, name the population swept and the instrument that swept it.** A superlative with a stated instrument is a finding; the same superlative without one is a guess wearing a finding's clothes. **Not SEP-specific** — it governs canon sweeps, ID-collision checks and `data/*.json` integrity work identically, and it is the discipline under which the **`canon-gen-001` dangling-citation flag** was recorded on 2026-08-10 (cited as binding authority in four spec bodies, absent from `data/canons.json`, three instruments swept, reported as *not found in the corpus* rather than *never ratified*). Related to but distinct from `textual-merge-cleanliness-does-not-imply-semantic-merge-cleanliness-on-structured-data` — that one is about choosing the right axis to check; this one is about what a clean check entitles you to say. **This is the first Cluster-B-originated entry, so it also decides the Maintenance §note's open question of SEP eligibility: eligible.**)
- **rendered-output-verification-is-a-distinct-jurisdiction-from-source-verification** (Cluster B 2026-08-10; carried here as cross-cluster. A PDF rasteriser (`poppler-utils` + PyMuPDF) was wired into both SEP repos' SessionStart hooks; **before it, "the file was written" was the only check that had ever run on a generated report**, and two artifacts had already shipped to the client under an explicit *"neither PDF has been looked at by a human or a renderer."* **The first look paid twice:** a refuted claim sitting in the body text of a sheet already sent, and **four of six days' caveats written, computed and thrown away** — a `note` variable unpacked and never emitted, all four strings returning zero matches in the shipped output. Compounding: the formatter printed one glyph for both *unmeasured* and *zero*, so a genuinely unmeasured cell was indistinguishable from a nil one row away. **Neither defect is detectable from the output totals; both are obvious on sight.** Rule: auditing the generator is not auditing the artifact — render it and look. **Direct Codex analog:** `split/build.sh` output discipline and Canon 0033/0034 already encode this instinct for build artifacts; the ledger now has the general form, and `docs/` PDF/HTML deliverables have no equivalent gate.)
