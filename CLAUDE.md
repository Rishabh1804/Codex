...
- **Clusters:** A = Codex + SproutLab + MSc (Censor: Cipher; MSc enrolled 2026-05-19 per canon-inst-004 on censorial-absorbability grounds). B = SEP Invoicing + SEP Dashboard + SEP-Ops/`soma-internal` (Censor: Nyx — **but SEP-Ops's own charter names Cipher; conflict open**). SEP-Ops admitted 2026-09-01, decree 0022. C = Command Center + Temple of Mars, the Monument class (Censor: Cipher — **INTERIM since 2026-05-24 per canon-inst-003, seat open for a successor**; "Monument" persists as a Province-class attribute, not a cluster). D = MyFin, Finance — **Censor VACANT** (ratified 2026-08-26, decree 0020, alongside the Cluster C propagation). **Two open censorial seats against one Censor stretched across two Clusters.** **Corporate parallel:** Studio (A) / SEP (B) / Monument Bloc (C) / Finance (D).
